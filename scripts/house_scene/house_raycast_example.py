"""
replica_rl_locomotion_lidar.py
-------------------------------
Your working replica_rl_locomotion.py with the native Genesis LiDAR
sensor added. Three additions only — everything else is identical:

  1. gs.sensors.Lidar added after robot, before scene.build()
  2. lidar.read() called in the loop
  3. scene.draw_debug_spheres() draws coloured hit points in viewer

Ray colours:
    Red    — obstacle < 1.0m
    Yellow — obstacle 1.0 - 2.0m
    Green  — obstacle > 2.0m

Usage:
    python replica_rl_locomotion_lidar.py \
        --checkpoint runs/go2_walk/checkpoint_final.pt \
        --spawn-x 1.0 --spawn-y -2.0
"""
...
GO2_SPAWN_POS  = [1.0, 1.0, 0.45]
GO2_SPAWN_QUAT = [0.0, 0.0, 0.0, 1.0]
...
# LiDAR thresholds for colour coding
CLOSE_THRESH  = 1.0
MEDIUM_THRESH = 2.0
MAX_RANGE     = 4.0
...
def load_policy(checkpoint_path: str, device: str) -> ActorCritic:
    ...
    print(f"  Policy loaded — hidden={hidden}  step={ckpt.get('step', '?'):,}")
    return policy


# ==========================================================================
#  Scene loading (unchanged)
# ==========================================================================
...
def spawn_entity(scene, template_name, asset_path, pos, quat, is_fixed, scale=1.0):
    try:
...
def build_scene(scene_json: str, asset_root: str, scene) -> None:
    ...
    for obj in config.get("object_instances", []):
        ...
        if asset:
            np_, nq = convert_habitat_to_genesis(obj["translation"], obj["rotation"])
...
def get_obs(robot, motor_dofs, commands, last_actions, device):
    base_quat   = robot.get_quat()
    inv_q       = inv_quat(base_quat)
    ang_vel     = transform_by_quat(robot.get_ang(), inv_q)
    grav_w      = torch.tensor([[0., 0., -1.]], device=base_quat.device)
    proj_grav   = transform_by_quat(grav_w, inv_q)
...
def main():
    ...
    parser.add_argument("--asset-root", default="data/replica_cad/")
    parser.add_argument("--vx",      type=float, default=0.4)
    parser.add_argument("--vy",      type=float, default=0.0)
    parser.add_argument("--vyaw",    type=float, default=0.0)
    parser.add_argument("--spawn-x", type=float, default=GO2_SPAWN_POS[0])
    parser.add_argument("--spawn-y", type=float, default=GO2_SPAWN_POS[1])
    parser.add_argument("--headless", action="store_true")
    # NEW: how often to update LiDAR visualisation (every N steps)
    parser.add_argument("--lidar-every", type=int, default=5)
    ...
    scene = gs.Scene(
        ...
        viewer_options = gs.options.ViewerOptions(
            camera_pos    = (args.spawn_x + 2.0, args.spawn_y - 2.0, 1.5),
            camera_lookat = (args.spawn_x, args.spawn_y, 0.4),
            camera_fov    = 50,
            max_FPS       = 120,
        ),
    ...
    # ── ADDITION 1: LiDAR sensor ──────────────────────────────────────────
    # Added after robot entity, before scene.build()
    print("\n── LiDAR sensor ──")
    lidar = None
    try:
        pattern = gs.sensors.SphericalPattern(
            fov         = (360.0, 20.0),
            n_points    = (36, 4),         # 36 rays, single horizontal layer
        )
        ...
        print(f"  ✅  LiDAR: 4 layers × 36 rays = 144 rays, max {MAX_RANGE}m")
    except Exception as e:
        print(f"  ⚠️  LiDAR unavailable: {e}")
    # ─────────────────────────────────────────────────────────────────────
    ...
    robot.set_dofs_position(init_pos, dofs_idx_local=motor_dofs, zero_velocity=True)
    ...
    if lidar is not None:
        print(f"LiDAR rays: 🔴<{CLOSE_THRESH}m  "
              f"🟡<{MEDIUM_THRESH}m  🟢<{MAX_RANGE}m")
    print("Close the viewer window to exit.\n")

    step = 0
    while True:
        # ── Policy step (unchanged) ────────────────────────────────────────
        ...
        robot.control_dofs_position(target_pos, dofs_idx_local=motor_dofs)
        scene.step()
